...
% Unfortunately, a single string will be a 'char array' while multiple string
% will come as a cell array, so to make sure we can live with both, we check and
% make it a cell array if it is not already.
if iscell(names)
    names_c = names;
else
    names_c{1} = names;
