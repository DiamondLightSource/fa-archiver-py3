...
/* Used to get the time this block was read.  This is close enough to the
 * completion of the FA sniffer read to be a good timestamp for the last frame.
 * It might be better for the driver to take account of this as it's closer to
 * the true event time. */
...
static void *sniffer_thread(void *context)
{
    ...
    while (true)
    {
        ...
        while (sniffer_ok)
        {
            ...
            sniffer_ok = sniffer_context->read(buffer, fa_block_size);
            ...
            IGNORE(TEST_OK_(release_write_block(
                fa_block_buffer, !sniffer_ok, get_timestamp()),
